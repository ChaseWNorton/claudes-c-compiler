...
pub struct LivenessResult {
    ...
    /// Per-register clobber points for caller-saved registers on i686.
    /// Index 0 = ecx clobber points (Store, Load, GEP, shifts, division, float cmp, intrinsics).
    /// Index 1 = edx clobber points (division, indirect stores, 64-bit casts).
...
fn assign_program_points(
    ...
    for (block_idx, block) in func.blocks.iter().enumerate() {
        ...
        for inst in &block.instructions {
            ...
            // Track per-register scratch clobber points (i686).
            // ecx: indirect memory access, shift counts, division, float cmp, intrinsics.
            // edx: division (edx:eax pair), indirect stores (emit_save_acc), 64-bit casts.
            if is_32bit {
                match inst {
                    // Store through non-alloca pointers: ecx for addressing, edx for save_acc
                    ...
                    // 64-bit casts: edx used for eax:edx pair (cltd, xorl %edx, etc.)
                    Instruction::Cast { from_ty, to_ty, .. }
                        if (matches!(from_ty, IrType::I64 | IrType::U64) || matches!(to_ty, IrType::I64 | IrType::U64))
                            && !matches!((from_ty, to_ty), (IrType::I64, IrType::U64) | (IrType::U64, IrType::I64)) =>
                    {
                        edx_clobber_points.push(point);
                    }
